...
impl ContentLabels {
    fn try_wrap<'de, D>(self, content: Content) -> std::result::Result<Content, D::Error>
    where
        D: Deserializer<'de>,
    {
        ...
        impl<'de> Deserialize<'de> for Content {
            ...
            D: Deserializer<'de>,
            {
                ...
                enum __ContentOrRef {
                    ...
                    Content(__ContentWithLabels),
                }

                match __ContentOrRef::deserialize(deserializer)? {
                    __ContentOrRef::Ref(ref_) => Ok(Self::SameAs(SameAsContent { ref_ })),
                    __ContentOrRef::Number(number) => {
                        let number_node = if number.is_u64() {
                            number_content::U64::from(number.as_u64().unwrap()).into()
                        } else if number.is_i64() {
                            number_content::I64::from(number.as_i64().unwrap()).into()
                        } else {
                            number_content::F64::from(number.as_f64().unwrap()).into()
                        };
                        Ok(Self::Number(number_node))
                    },
                    $(
                        __ContentOrRef::Content(__ContentWithLabels {
                            labels,
                            content: __Content::$name(inner)
                        }) => {
                            let inner_as_content = Content::$name(inner);
                            labels.try_wrap::<D>(inner_as_content)
                        },
                    )+
                }
            }
